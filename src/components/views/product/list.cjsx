...
ListView = React.createClass
      ...
      <div className="row" ref="container" style={opacity: 0}>
        <div className="col s12">
          <div className="collection with-header">
            <div className="collection-header"><h4>Products</h4></div>
  ...
  componentDidUpdate: (props, state) ->
    $('.product-item').velocity 'transition.slideRightIn', stagger: 10
  ...
  renderItemLink: (id) ->
    ...
    (
      <Link className="collection-item product-item" key={id} to="item" params={{id: id}} style={opacity: 0, width: '100%'}>{item.title}</Link>
    )
